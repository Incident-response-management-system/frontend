...
import { fetchAgencyIncidents, updateIncidentStatus, fetchAgencyStats, updateAgencyProfile, type IncidentTab } from '@/lib/agency-api';
...
function SettingsTab({ onProfileSaved }: { onProfileSaved: (p: AgencyUser) => void }) {
  const isMobile = useIsMobile();
  const profile = useAgencyProfile();
  const typeLabel = profile?.agencyType ? (AGENCY_TYPE_LABELS[getAgencyTypeKey(profile.agencyType)] || profile.agencyType) : '';
    ...
    <div>
      ...
      <div style={{ padding: isMobile ? 16 : 32, maxWidth: 720 }}>
        <div style={{ background: 'var(--brand-white)', border: '1px solid var(--brand-hairline)', borderRadius: 12, padding: isMobile ? 20 : 28 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--brand-muted)', margin: '0 0 24px' }}>Update your agency details, service radius and base location.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <FormInput label="Agency name" value={agencyName} onChange={e => setAgencyName(e.target.value)} />
            ...
            <div>
              ...
              <input type="range" min="1" max="500" value={radius} onChange={e => setRadius(Number(e.target.value))} style={{ width: '100%', accentColor: 'var(--status-red)' }} />
            </div>
            <div>
