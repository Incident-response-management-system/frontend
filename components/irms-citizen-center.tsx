          ...
          <div style={{ background: 'var(--brand-white)', border: '1px solid var(--brand-hairline)', borderRadius: 10, overflow: 'hidden' }}>
            {[
              ...
              <div key={i} style={{ padding: '12px 16px', borderBottom: i < arr.length - 1 ? '1px solid var(--brand-hairline)' : 'none', display: 'flex', gap: 12, fontSize: 13, color: 'var(--brand-ink)' }}>
                <span style={{ width: 16, height: 16, borderRadius: '50%', border: '1px solid var(--brand-divider)', flexShrink: 0, marginTop: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icon.check style={{ width: 10, height: 10, color: 'var(--brand-muted)' }} />
                </span>
                {t}
              </div>
            ))}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', maxWidth: 420, width: '100%', margin: '40px auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <button onClick={submit} style={{
            background: 'var(--brand-ink)', color: 'var(--brand-cream)', padding: '12px 24px', borderRadius: 9,
            fontWeight: 600, fontSize: 14, marginTop: 4, border: 'none', cursor: 'pointer'
...
export function CitizenLoginScreen({ navigate, onAuth }: CitizenAuthProps) {
  ...
  const submit = () => {
    ...
    toast.success('Welcome back, Chinedu!');
    navigate('my-reports');
  };
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        border: `1px solid ${focused ? 'var(--brand-ink)' : 'var(--brand-hairline)'}`,
        ...
        />
        {isPassword && (
          <button onClick={() => setShow(!show)} style={{ color: 'var(--brand-muted)', padding: 4, background: 'none', border: 'none', cursor: 'pointer' }}>
...
const MY_REPORTS: Incident[] = [
  { ref: 'INC-2026-00149', type: 'medical', location: 'Auditorium 3, Main Bowl', status: 'received', reportedAt: 'Today · 14:32', desc: 'Elderly man collapsed during service.', media: 2, assignedTo: null, lat: 6.8932, lng: 3.1721, reported: '2 min ago' },
  { ref: 'INC-2026-00131', type: 'rta', location: 'Lagos-Ibadan Expressway, Mile 46', status: 'assigned', reportedAt: '23 May · 18:12', desc: 'Two-vehicle collision near camp gate.', media: 3, assignedTo: 'Federal Road Safety Corps', lat: 6.8865, lng: 3.1812, reported: '5 days ago' },
  { ref: 'INC-2026-00114', type: 'flood', location: 'Drainage Channel B', status: 'resolved', reportedAt: '18 May · 09:40', desc: 'Water overflow blocking pedestrian path.', media: 1, assignedTo: 'Camp Maintenance Unit', lat: 6.8878, lng: 3.1656, reported: '10 days ago' },
    ...
    <div style={{ background: 'var(--brand-cream)', minHeight: '100vh', color: 'var(--brand-ink)' }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
          <button onClick={() => navigate('landing')} style={{ background: 'none', border: 'none', cursor: 'pointer' }}><IRMSLogo size={15} color="var(--brand-ink)" /></button>
          <nav style={{ display: 'flex', gap: 6 }}>
            <button style={{ padding: '8px 12px', fontSize: 13, fontWeight: 600, color: 'var(--brand-ink)', borderRadius: 7, background: 'var(--brand-divider)', border: 'none', cursor: 'pointer' }}>My Reports</button>
            <button onClick={() => navigate('report')} style={{ padding: '8px 12px', fontSize: 13, color: 'var(--brand-muted)', borderRadius: 7, background: 'none', border: 'none', cursor: 'pointer' }}>Report new</button>
          </nav>
        </div>
        <div style={{ position: 'relative' }}>
          <button onClick={() => setProfileOpen(!profileOpen)} style={{
            ...
            <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--brand-divider)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600 }}>
          ...
          {profileOpen && (
            <>
              ...
              <div style={{
                ...
                minWidth: 220, padding: 6, boxShadow: '0 12px 28px rgba(0,0,0,0.12)',
              }}>
                <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--brand-hairline)', marginBottom: 4 }}>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{user?.name}</div>
                  <div style={{ fontSize: 11, color: 'var(--brand-muted)' }}>{user?.email}</div>
                </div>
      ...
      <div style={{ maxWidth: 1120, margin: '0 auto', padding: '40px 32px 80px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32, gap: 24, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => navigate('report')} style={{
            background: 'var(--brand-ink)', color: 'var(--brand-cream)', padding: '10px 16px', borderRadius: 9,
        ...
        <div style={{
          ...
          {[
            ...
            { l: 'Under review', v: MY_REPORTS.filter(r => r.status === 'review' || r.status === 'assigned').length, c: 'var(--status-amber)' },
            { l: 'Resolved', v: MY_REPORTS.filter(r => r.status === 'resolved').length, c: 'var(--status-green)' },
          ].map((s, i) => (
            <div key={i} style={{ padding: '20px 24px', borderLeft: i > 0 ? '1px solid var(--brand-hairline)' : 'none' }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--brand-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{s.l}</div>
              <div style={{ fontSize: 28, fontWeight: 700, fontFamily: 'var(--font-mono)', letterSpacing: '-0.02em', color: s.c }}>{s.v}</div>
            </div>
          ))}
        </div>

        {/* Tabs */}
        <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid var(--brand-hairline)', marginBottom: 0 }}>
          {filters.map(f => (
            <button key={f.id} onClick={() => setTab(f.id)} style={{
              padding: '10px 16px', fontSize: 13, fontWeight: 600,
              color: tab === f.id ? 'var(--brand-ink)' : 'var(--brand-muted)',
              borderBottom: tab === f.id ? '2px solid var(--brand-ink)' : '2px solid transparent',
              ...
              <span style={{
                ...
                background: tab === f.id ? 'var(--brand-ink)' : 'var(--brand-cream)',
        ...
        <div style={{ background: 'var(--brand-white)', border: '1px solid var(--brand-hairline)', borderTop: 'none', borderRadius: '0 0 12px 12px', overflow: 'hidden' }}>
          {filtered.length === 0 ? (
            <div style={{ padding: '60px 24px', textAlign: 'center', color: 'var(--brand-muted)' }}>
              ...
              <button onClick={() => navigate('report')} style={{ fontSize: 13, fontWeight: 600, color: 'var(--brand-ink)', textDecoration: 'underline', textUnderlineOffset: 3, background: 'none', border: 'none', cursor: 'pointer' }}>Report your first incident</button>
            </div>
          ) : filtered.map((r, idx) => {
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--brand-muted)' }}>{r.ref}</span>
                    <span style={{ fontSize: 14, fontWeight: 600 }}>{t.label}</span>
                  </div>
                  <div style={{ fontSize: 13, color: 'var(--brand-muted)', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Icon.pin style={{ width: 12, height: 12, color: 'var(--brand-muted)' }} />
                    {r.location}
                    <span style={{ color: 'var(--brand-divider)' }}>·</span>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--brand-muted)' }}>{r.reportedAt}</span>
                  </div>
                </div>
...
function CitizenReportDetail({ report, onClose }: { report: Incident; onClose: () => void }) {
  const t = getIncidentType(report.type);
  const handleUpdate = () => {
    toast.success('Your update has been submitted to the responding agency!');
  };
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{ position: 'sticky', top: 0, background: 'var(--brand-white)', padding: '20px 28px 16px', borderBottom: '1px solid var(--brand-hairline)', zIndex: 5 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--brand-muted)', letterSpacing: '0.1em', marginBottom: 6 }}>REFERENCE</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 20, fontWeight: 700 }}>{report.ref}</div>
                <StatusBadge status={report.status} />
              </div>
            </div>
            <button onClick={onClose} style={{ width: 34, height: 34, borderRadius: 9, border: '1px solid var(--brand-divider)', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none', cursor: 'pointer' }}><Icon.close /></button>
          </div>
        </div>

        <div style={{ padding: '24px 28px 40px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', borderRadius: 10, background: 'var(--brand-cream)', border: '1px solid var(--brand-hairline)', marginBottom: 24 }}>
            <div style={{
              width: 36, height: 36, borderRadius: 8, background: 'var(--brand-white)', border: '1px solid var(--brand-divider)',
          ...
          <div style={{ padding: '4px 0 24px', borderBottom: '1px solid var(--brand-hairline)', marginBottom: 24 }}>
            <StatusStepper current={report.status} timestamps={{
              received: report.reportedAt.split('·')[1]?.trim() || '14:32',
          ...
          <DetailRow label="Location">
            ...
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--brand-muted)' }}>{report.lat.toFixed(4)}° N · {report.lng.toFixed(4)}° E</div>
          </DetailRow>

          <DetailRow label="Your description">
            <p style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--brand-ink)', margin: 0 }}>{report.desc}</p>
          </DetailRow>

          {report.assignedTo && (
            <DetailRow label="Responding agency">
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 2 }}>{report.assignedTo}</div>
              <div style={{ fontSize: 12, color: 'var(--brand-muted)' }}>Acknowledged · responding now</div>
            </DetailRow>
          )}
            ...
            <DetailRow label={`Evidence (${report.media})`}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {Array.from({ length: report.media }).map((_, i) => (
                  <div key={i} style={{ aspectRatio: 1, borderRadius: 8, background: `linear-gradient(135deg, oklch(0.4 0.08 ${i*60}), oklch(0.3 0.06 ${i*60+40}))`, border: '1px solid var(--brand-divider)' }}/>
                ))}
              </div>
            </DetailRow>
          )}
          ...
          {report.status !== 'resolved' && (
            <div style={{ marginTop: 24, padding: 16, borderRadius: 10, background: 'var(--brand-cream)', border: '1px solid var(--brand-divider)' }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--brand-ink)', marginBottom: 8 }}>Have an update?</div>
              <div style={{ fontSize: 12, color: 'var(--brand-muted)', marginBottom: 12, lineHeight: 1.5 }}>
                If the situation has changed on the ground, add a note. The responding agency will see it immediately.
              </div>
              <button onClick={handleUpdate} style={{
                padding: '9px 14px', borderRadius: 8, border: '1px solid var(--brand-divider)',
                fontSize: 13, fontWeight: 500, color: 'var(--brand-ink)', background: 'white', cursor: 'pointer'
...
function DetailRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--brand-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>{label}</div>
      {children}
    </div>
  );
}
